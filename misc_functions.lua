    ...
    if _card and G.STAGE == G.STAGES.RUN then
        ...
        if _card.area then
            ...
            if _card.area == G.pack_cards then
                ...
                --local is_tarot_use = _card.ability.set == 'Tarot' and (_card.config.center.key =='c_high_priestess' or _card.config.center.key =='c_hermit')
                ...
                local is_card_can_be_used = is_zodiac or is_planet or is_auxiliary_use --or is_tarot_use --or is_silly_use
                local is_mod_compact = is_horoscope or is_auxiliary or is_zodiac or is_collectable or is_alchemical
                ...
                if is_consumeable_card_in_crazy_reverie_pack then
                            drag_target({
                                ...
                                colour = adjust_alpha(--[[(is_card_can_be_used and G.C.RED) or]] G.C.GREEN, 0.9),
                                text = { --[[(is_card_can_be_used and localize('b_use')) or]] localize('b_select') },
                                card = _card,
                                active_check = (function(other)
                                    if is_card_can_be_used and G.consumeables.config.card_limit > #G.consumeables.cards then
                                        return other:can_use_consumeable()
                                    elseif (G.consumeables.config.card_limit > #G.consumeables.cards) or (_card.ability.set == 'Cine') or is_collectable or (is_horoscope and G.mxms_horoscope.config.card_limit > #G.mxms_horoscope.cards) then
                ...
                -- rewrite this mess
                    if _card.ability.consumeable then
                        local draw_use_drag_target = function()
                            drag_target({
                                cover = ((is_planet or is_mod_compact or is_spectral) and G.DRAG_TARGETS.P_select) or G.DRAG_TARGETS.C_use,
                                colour = adjust_alpha(((is_planet or ((is_mod_compact or is_spectral) and not is_zodiac and not is_auxiliary_use)) and G.C.GREEN) or G.C.RED, 0.9),
                                text = { (((is_mod_compact and not is_zodiac and not is_auxiliary_use) or is_spectral) and localize('b_select')) or localize('b_use') },
                                card = _card,
                                active_check = (function(other)
                                    if ((is_alchemical or is_auxiliary or (is_spectral and not is_horoscope)) and G.consumeables.config.card_limit > #G.consumeables.cards) or (is_horoscope and G.mxms_horoscope.config.card_limit > #G.mxms_horoscope.cards) or is_collectable then
                                        return sticky_can_select_card(other)
                                ...
                                release_func = (function(other)
                                    if other:can_use_consumeable() then
                                        G.FUNCS.use_card({ config = { ref_table = other } })
                                elseif is_alchemical and sticky_can_select_card(other) then
                                      G.FUNCS.use_card({ config = { ref_table = other } })
                                    elseif (is_alchemical or is_auxiliary or is_collectable or is_spectral or is_horoscope) and sticky_can_select_card(other) then
                                        G.FUNCS.take_card({ config = { ref_table = other } })
                                    end
                                end)
                            })
